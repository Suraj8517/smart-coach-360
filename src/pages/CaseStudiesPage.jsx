...
const studies = [
  { type:"Personal Trainer", emoji:"🏋️", quote:"I Went From 12 Clients to 35 in 6 Months Without Burning Out", subheading:"Independent Personal Trainer — Online & In-person",
  ...
  { type:"Gym Owner", emoji:"🏢", quote:"SmartCoach360 Gave Me Real Visibility Across All 3 Gyms for the First Time", subheading:"Fitness Studio Owner — 3 Locations, 8 Coaches",
    ...
    <div className="pt-16">
      <section className="relative py-32 overflow-hidden">
        ...
        <div className="relative z-10 max-w-3xl mx-auto px-4 text-center">
          ...
          <p className="text-brand-silver text-lg leading-relaxed">Here's how fitness professionals are using SmartCoach360 to build better businesses and deliver better results for their clients.</p>
        </div>
      </section>

      <section className="bg-brand-lilac">
        {studies.map((cs, i) => (
          <div key={i} className={`py-24 ${i % 2 === 0 ? "bg-brand-lilac" : "bg-brand-lilac-100"}`}>
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
              <div className={`grid lg:grid-cols-2 gap-14 items-start ${i % 2 !== 0 ? "lg:grid-flow-col-dense" : ""}`}>
                  ...
                  <div className="grid grid-cols-5 gap-2">
                    {cs.results.map((r,j) => (
                      <div key={j} className="rounded-2xl p-3 text-center" style={DARK_BG}>
                        ...
                        <p className="text-brand-silver text-xs mt-0.5 leading-tight">{r.sub}</p>
                      </div>
                    ))}
                  </div>
                </div>
                <div className={i % 2 !== 0 ? "lg:col-start-1" : ""}>
                  ...
                  <blockquote className="font-display text-2xl sm:text-3xl text-brand-black mb-7 leading-tight border-l-4 pl-5" style={{ borderColor:"#47294c" }}>
                    "{cs.quote}"
                  </blockquote>
                  ...
                  <div className="mt-8 flex items-center gap-4 bg-brand-lilac border border-brand-silver-xl rounded-2xl p-4">
                    <img src={cs.avatarImg} alt="Coach" className="w-12 h-12 rounded-2xl object-cover flex-shrink-0" />
                    <div>
      ...
      <section className="py-20 relative overflow-hidden" style={DARK_BG}>
        <div className="absolute inset-0" style={{ backgroundImage:`url(${IMAGES.running})`, backgroundSize:"cover", backgroundPosition:"center", opacity:0.12 }} />
        <div className="relative z-10 max-w-6xl mx-auto px-4">
          <SectionHeader tag="Platform Results" title="The Numbers Speak for Themselves" light />
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
            {[{val:"10,000+",label:"Clients managed on platform"},{val:"40%",label:"Average retention improvement"},{val:"12hrs",label:"Admin saved per week, per coach"},{val:"99%",label:"Payment collection rate"}].map((m,i)=>(
              <div key={i} className="rounded-3xl p-7 border text-center" style={{ backgroundColor:"rgba(71,41,76,0.4)", borderColor:"rgba(92,54,99,0.5)" }}>
                <p className="font-display text-5xl text-brand-lilac-300 mb-2">{m.val}</p>
                <p className="text-brand-silver text-sm leading-relaxed">{m.label}</p>
              </div>
            ))}
          </div>
        </div>
